...
> gmake TARGET=example-Read-DelayedCoincidenceEvent-Muon-LikelihoodRatio-XePhase

2. select candidate events 
w/o "cout"
> ( ./example-Read-DelayedCoincidenceEvent-Muon-LikelihoodRatio-XePhase.Linux) >& ./log/DelayedCoincidence-Muon-XePhase.log-121213 &
...
#if 0
  // dcvf list
  char dcvf_list_input_filename[256] = "./analysis_list/muon-dcvf-A2-XePhase.list-121207";
  
  // output file
  char root_output_filename[256]     = "./result/DelayedCoincidence-Muon-XePhase_121207.root";
#endif


#if 1
  // dcvf list
  char dcvf_list_input_filename[256] = "./analysis_list/muon-dcvf-A2-XePhase.list"; 
  
  // output file
  char root_output_filename[256]     = "./result/DelayedCoincidence-Muon-XePhase-11947.root"; // run 10676 - 11947
#endif
  ...
  while (flist >> run >> file) {
	...
	CorrectionFactor = R_balloon / (p0 * exp(p1 * energy) + p2);
	 ...
	 event.Delayed.EnergyA2 <1.0
      ...
      // fiducial, time, space and cylinder cut, Xe cut
      //      if(Rp>Fiducial || Rd>Fiducial || dt>dTCut || dl>dRCut || dt<0.5) continue;
      //      if(dt>dTCut || dl>dRCut || dt<0.5) continue; // orginal
      if(Rp>Fiducial || Rd>Fiducial || Rd<250 || dt>dTCut || dl>dRCut || dt<0.5) continue;

      // cylinder cut for delayed events
      if (Zd > 0 && Rxyd < 250) continue;
