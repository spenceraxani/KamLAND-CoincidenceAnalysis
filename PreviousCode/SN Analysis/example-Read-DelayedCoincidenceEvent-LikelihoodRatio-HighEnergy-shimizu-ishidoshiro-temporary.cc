...
int main(int argc, char* argv[])
{
...
#if 1
  //string ListFile  = "./analysis_list/dcvf-Kat.list-GW";
  //string ListFile  = "./analysis_list/dcvf-Kat.list-GW-accidental_woballon";
  //string ListFile  = "./analysis_list/dcvf-Kat.list-GW-accidental-with";
  //string Reactor = "./result/Observed-dcvf-Kat-GW-accidental.dat";
  
  string ListFile  = "./analysis_list/dcvf-Kat.list-preSN";
  string Reactor = "./result/Observed-dcvf-Kat-preSN.dat";
  

  char root_output_filename[256]                       = "./result/DelayedCoincidence_GW.root";
  char root_output_filename_likelihood_selection[256] = "./result/DelayedCoincidence_LikelihoodSelection_GW.root";
  string Rungrade = "./result/Observed-dcvf-Kat-rungrade.dat";
  string Runlist = "./result/Observed-dcvf-Kat-runlist.dat";
#endif
  ...
  // ------------------------------------------------------------------------------------------ //
   ...
   ofstream fRunlist(Runlist.c_str());
  ...
  float Ed, dR, dT, dTp, Ep, xp, yp, zp, xd, yd, zd, rp, rd, Evis;
 ...
 while (flist >> file) {
    ...
    */
	...
	abort();
      ...
      fRunlist << RunNumber  <<  " " << run_grade << " " << endl;  
...
//       rd = Rd;
      ...
      //if (dl >= 200 || dt < 2000 || dt >= 1002000) continue;
      ...
      //if (energy >= 0.9 && energy < 100.0 && N200OD < 5 && multiplicity <= 1) {
      if (energy >= 0.9 && energy < 100.0 && N200OD < 5) {
      //if (energy >= 0.9 && energy < 100.0 &&  multiplicity <= 1) {
	...
	//}
	  ...
	  double Likelihood_Ratio_Cut = KVFLikelihoodRatio::Likelihood_Ratio_Cut(); 
	  //if (Likelihood_Ratio > KVFLikelihoodRatio::Likelihood_Ratio_Cut()) {
	    

	  //fReactor << run << " " <<  energy << " " <<  event.Prompt.UnixTime << " " << event.Prompt.EventNumber << " " << event.Delayed.EventNumber << " " << xp << " "  << yp << " " << zp << " " << xd << " " << yd << " " << zd << " "<< run_grade << " " << multiplicity << " " << dt << " " << dl << " " << endl;	
	  ...
	  printf("                %8.2f  %8.2f  %8.2f\n",
		 ...
		 event.Delayed.Y,
		 event.Delayed.Z);
	  cout << "Likelihood Ratio" <<  " (Measured)" << Likelihood_Ratio << " (Creiteria)" <<  KVFLikelihoodRatio::Likelihood_Ratio_Cut()  << endl;
	  cout << "Rp, Rd" <<  " " << Rp << " " << Rd  << endl;
	  cout << "Muon veto" << " " << event.Veto[0] << " " << event.Veto[1] << " " << event.Veto[2] << " " << event.Veto[3] << " " << event.Veto[4] << " " << event.Veto[5] << " "<< endl;
	  printf("\n");
	  
	  fReactor << run << ", " <<  event.Prompt.EventNumber << ", " <<event.Prompt.TimeStamp << ", "<< event.Prompt.UnixTime << "," << prompt << veto << ", "  << Ep << ", " << Ed << ", " << dR << ", " << dT << ", " << dTp << ", " << xp << ", " << yp << ", " << zp << ","<< xd << ", " << yd << ", " << zd << ", "<< Evis << ", " << vsTp << ", " << chiQp << ", " << chiTp <<", " << chiQd << ", " << chiTd << ", " << NhitOD << ", " << N200OD << ", " << totalQ17 << ", " << totalQOD << ", " << VertexBadness << ", " << multiplicity << ", " << event.Veto[0] << ", " << event.Veto[1] << ", " << event.Veto[2] << ", "  << event.Veto[3] << ", " << event.Veto[4] << ", " << event.Veto[5] << ", " <<  Likelihood_Ratio << ", " << Likelihood_Ratio_Cut << endl;
