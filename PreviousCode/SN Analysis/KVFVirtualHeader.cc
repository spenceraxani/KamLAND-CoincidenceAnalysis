...
bool KVFVirtualHeader :: isBigEndian(void)
{
...
unsigned long long int KVFVirtualHeader :: swap(unsigned long long int value)
{
...
bool KVFVirtualHeader :: restore(unsigned long long int* L, const char* buf,int whence)
{
...
bool KVFVirtualHeader :: store(unsigned long long int L, char* buf, int whence)
{
  if( isBigEndian() ){}
  else {
    L = swap(L);
