...
int main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < (argc - 1); i++) {
    ...
    for(int p=0;p<n;p++){
      DelayedEvent[p].enablePositionDependentEnergyCorrection();
    }
...
//  	    else if( dQ>=1000000) muon_veto4++;
	...
	int NDeads = deadtime_manager2.getNofDeadTime();
	for (int did = 0; did < NDeads; did++) 
	    cout << deadtime_manager2.getDeadTimeStart(did) << " " << deadtime_manager2.getDeadTimeStop(did) << endl;
