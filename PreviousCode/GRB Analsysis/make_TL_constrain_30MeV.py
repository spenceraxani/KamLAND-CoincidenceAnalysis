...
def cal_number_event(T,L,zList1,zList3):
    ...
    #for line in range(1):
        ...
        hoge1 = e1 * np.sum(NList[np.where( (EnuList > 7.5+0.78) & (EnuList< 100+0.78))])
        hoge2 = e1 * np.sum(NList[np.where( (EnuList > 7.5+0.78) & (EnuList< 30+0.78))])
        #print "Expected event %s w/o resokution" %hoge
        Nevent.append(hoge1)
        Nevent2.append(hoge2)
    ...
    for line in range(len(zList3)):
        ...
        hoge = eff*NList
        hoge1 = hoge[np.where( (EnuList > 0.9+0.78) & (EnuList< 100+0.78))]
        hoge2 = hoge[np.where( (EnuList > 0.9+0.78) & (EnuList< 30+0.78))]

        Nevent.append(np.sum(hoge1))
        Nevent2.append(np.sum(hoge2))
...
if __name__ == "__main__":
    ...
    for line1 in range(1,30):
        ...
        print "Upper limits on L=%s, %s (Ep<100MeV, Ep<30MeV)" %(Lupper,Lupper2)
